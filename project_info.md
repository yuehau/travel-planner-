...
- Added price-tiered plan templates (`src/data/plans.ts`): each available state offers Plan A/B/C (Budget/Balanced/Premium), a curated attraction subset with cost and day count computed live from `routeBuilder`. Picking a plan pre-fills the Explore cart and jumps straight to the editable route view; users can still remove/add stops before creating the trip, or skip plans entirely and browse every attraction themselves.
- Added a Weather & Travel Conditions panel at the top of the Essentials tab (`WeatherAndTravelInfo.tsx`): a free, keyless 7-day forecast from Open-Meteo (`src/services/weather.ts`), keyed off the average coordinates of the trip's saved places. Real-time traffic data needs a paid API (Google Maps/TomTom), so traffic is static, destination-matched tips instead (`src/data/travelTips.ts` - Penang/Melaka-specific, generic fallback otherwise).
- Replaced the email-invite Sharing tab with a **Group Trip** tab (`GroupTripView.tsx`) and a new `/groups` browse page (`GroupTripsPage.tsx`): a trip owner can open their trip to a group with a target headcount instead of inviting people one by one; other signed-in users browse open trips, preview their saved places, and self-join with one click (no approval step) until capacity is reached. Required loosening RLS on `trips`/`places`/`itinerary_items`/`itinerary_nodes`/`trip_members` so an open trip's list is readable by non-owners (`trip_is_open_group()` helper + a new self-join INSERT policy) - editing a trip's itinerary/budget/etc. is still owner-only, only the read path and the self-join insert were opened up. Capacity is enforced at the app layer (button disabled once full), not as a hard concurrent-safe DB constraint - acceptable for this prototype.
