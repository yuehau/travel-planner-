...
const WeatherAndTravelInfo: React.FC<WeatherAndTravelInfoProps> = ({ tripId, travelData, trip }) => {
  ...
  return (
    <section className="mb-10 p-6 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
      ...
      {isLoading && <p className="text-sm text-zinc-500 dark:text-zinc-400">Loading forecast...</p>}
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 mb-5">
          {forecast.map((day) => (
            <div
              ...
              className="shrink-0 w-24 text-center rounded-xl bg-white dark:bg-[#0a0a0a] border border-zinc-200 dark:border-zinc-800 p-3"
            >
              ...
              <div className="text-[10px] text-zinc-400 dark:text-zinc-500 mt-1">{day.precipitationChance}% rain</div>
            </div>
          ))}
        </div>
      )}

      <div className="flex items-start gap-2 pt-4 border-t border-zinc-200 dark:border-zinc-800">
        <Car size={16} className="mt-0.5 shrink-0 text-zinc-400 dark:text-zinc-600" />
