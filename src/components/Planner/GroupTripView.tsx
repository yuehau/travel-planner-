  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-display font-bold tracking-tighter">Group Trip</h2>
      ...
      <form onSubmit={handleSave} className="mb-10 grid gap-4 rounded-2xl border border-stone-200 bg-stone-50 p-5 dark:border-stone-800 dark:bg-stone-900">
        <label className="flex items-center justify-between gap-4 text-sm font-medium">
          ...
          <input
            ...
            className="h-5 w-5 accent-stone-900 dark:accent-stone-100"
          />
        </label>
        ...
        <button
          type="submit"
          disabled={isSaving || travelData.isReadOnly}
          className="w-fit px-5 py-2.5 rounded-xl bg-coral-500 text-white text-sm font-medium hover:opacity-90 transition-opacity disabled:cursor-not-allowed disabled:opacity-50"
        >
      ...
      {isLoading ? (
        <div className="py-12 text-center text-stone-500 dark:text-stone-400">Loading group...</div>
      ) : acceptedMembers.length === 0 ? (
        <div className="py-12 text-center text-stone-500 dark:text-stone-400">Nobody has joined yet.</div>
      ) : (
        <div className="space-y-3">
          {acceptedMembers.map((member) => (
            <article
              key={member.id}
              className="flex items-center justify-between rounded-2xl border border-stone-200 bg-stone-50 p-4 dark:border-stone-800 dark:bg-stone-900"
            >
              <div>
                <p className="font-medium">{member.invited_email ?? 'A fellow traveler'}</p>
                <p className="text-sm text-stone-500 dark:text-stone-400">Joined the group</p>
              </div>
              {!travelData.isReadOnly && (
                <button
                  type="button"
                  ...
                  className="p-1 text-stone-300 dark:text-stone-700 hover:text-red-500 transition-colors"
                >
                  <X size={18} />
                </button>
              )}
