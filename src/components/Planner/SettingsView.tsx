...
const SettingsView = ({ travelData }: SettingsViewProps) => {
  ...
  if (isLoading) {
    return <div className="py-12 text-center text-sand-500 dark:text-sand-400">Loading settings...</div>;
  }

  if (!settings) {
    return <div className="py-12 text-center text-sand-500 dark:text-sand-400">Settings unavailable.</div>;
  }
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-sand-500 dark:text-sand-400">Defaults used across your planning workspace.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid gap-4 rounded-2xl border border-sand-200 bg-sand-50 p-5 dark:border-sand-800 dark:bg-sand-900">
        <label className="grid gap-1 text-sm font-medium">
          ...
          <input
            ...
            className="rounded-xl border border-sand-200 bg-white px-4 py-2 outline-none ring-clay-500 focus:ring-2 dark:border-sand-800 dark:bg-sand-950 dark:ring-clay-400"
          />
        </label>
        ...
        <button
          type="submit"
          disabled={isSaving || travelData.isReadOnly}
          className="inline-flex w-fit items-center gap-2 rounded-full bg-clay-600 px-4 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-50 dark:bg-clay-500 dark:text-white"
        >
          <Save size={16} />
          Save Preferences
        </button>
