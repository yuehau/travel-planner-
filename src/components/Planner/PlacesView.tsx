    ...
    <div className="mx-auto max-w-5xl px-4 py-8">
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-sand-500 dark:text-sand-400">Search, save, and pin the places that shape this trip.</p>
        </div>
        <form onSubmit={handleSearch} className="flex w-full gap-2 md:max-w-md">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-full border border-sand-200 bg-sand-50 py-2 pl-10 pr-4 text-sm outline-none ring-clay-500 transition-all focus:ring-2 dark:border-sand-800 dark:bg-sand-900 dark:ring-clay-400"
            />
          </div>
          <button
            type="submit"
            disabled={isSearching}
            className="rounded-full bg-clay-600 px-4 py-2 text-sm font-medium text-white transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-clay-500 dark:text-white"
          >
      ...
      <div className="mb-8 h-[360px] overflow-hidden rounded-2xl border border-sand-200 dark:border-sand-800">
      ...
      {results.length > 0 && (
        <section className="mb-10">
          <h3 className="mb-3 text-xs font-bold uppercase tracking-widest text-sand-400 dark:text-sand-600">Search Results</h3>
          <div className="space-y-2">
            {results.map((result) => (
              <div key={result.place_id} className="flex items-start justify-between gap-4 rounded-xl border border-sand-200 bg-sand-50 p-4 dark:border-sand-800 dark:bg-sand-900">
                <div>
                  <p className="text-sm font-semibold">{result.name || result.display_name.split(',')[0]}</p>
                  <p className="mt-1 text-sm text-sand-500 dark:text-sand-400">{result.display_name}</p>
                </div>
                <button
                  type="button"
                  disabled={isAdding || travelData.isReadOnly}
                  onClick={() => handleSaveResult(result)}
                  className="rounded-full bg-clay-600 px-3 py-1.5 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-50 dark:bg-clay-500 dark:text-white"
                >
                  Save
                </button>
              </div>
      ...
      {isLoading ? (
        <div className="py-12 text-center text-sand-500 dark:text-sand-400">Loading places...</div>
      ) : places.length === 0 ? (
        <div className="py-12 text-center text-sand-500 dark:text-sand-400">No places saved yet.</div>
      ) : (
        <div className="grid gap-4 md:grid-cols-2">
          {places.map((place) => (
            <article key={place.id} className="rounded-2xl border border-sand-200 bg-sand-50 p-5 dark:border-sand-800 dark:bg-sand-900">
              <div className="mb-3 flex items-start justify-between gap-3">
                <div>
                  <h3 className="font-semibold">{place.name}</h3>
                  {place.address && <p className="mt-1 text-sm text-sand-500 dark:text-sand-400">{place.address}</p>}
                </div>
                <MapPin size={18} className="text-sand-400" />
              </div>
              <div className="flex items-center gap-3 text-xs text-sand-500 dark:text-sand-400">
