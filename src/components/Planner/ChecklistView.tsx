...
const ChecklistView: React.FC<ChecklistViewProps> = ({ tripId, travelData }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-4">
      ...
      {isLoading ? (
        <div className="py-12 text-center text-sand-500 dark:text-sand-400">Loading packing list...</div>
      ) : items.length === 0 ? (
        <div className="py-12 text-center text-sand-500 dark:text-sand-400">No packing items yet.</div>
      ) : (
        <div className="space-y-10">
          {categories.map((category) => (
            <div key={category}>
              <h3 className="text-xs font-bold uppercase tracking-widest text-sand-400 dark:text-sand-600 mb-4">{category}</h3>
              <div className="space-y-2">
                {items.filter((item) => item.category === category).map((item) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-3 rounded-xl bg-sand-50 dark:bg-sand-900 border border-sand-200 dark:border-sand-800 hover:border-sand-400 dark:hover:border-sand-600 transition-all text-left group"
                  >
                    <span className="flex items-center gap-3">
                      {item.is_packed ? (
                        <Check size={18} className="text-green-500" />
                      ) : (
                        <Circle size={18} className="text-sand-300 dark:text-sand-700 group-hover:text-sand-400" />
                      )}
                      <span className={`text-sm ${item.is_packed ? 'text-sand-400 dark:text-sand-600 line-through' : 'text-sand-700 dark:text-sand-300'}`}>
                        {item.item}
                      </span>
                    </span>
                  </button>
